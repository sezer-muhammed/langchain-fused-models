...
class MultiModelManager(BaseChatModel):
    ...
    def _invoke_with_fallback(
        ...
        while current_idx not in tried_models:
            ...
            try:
                logger.debug(
                    f"Attempting request with model {current_idx} (type: {model._llm_type})"
                )
                ...
                result = model._generate(messages=messages, **kwargs)
                ...
                if (
                    ...
                    and "token_usage" in result.llm_output
                ):
                ...
                logger.warning(f"Model {current_idx} failed with {type(e).__name__}: {str(e)}")
                ...
                if not self.default_fallback or not should_retry:
                    logger.error("Fallback disabled or error not retryable, re-raising exception")
                    raise
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.debug(
                f"Starting generation with model {model_idx} (type: {selected_model._llm_type})"
            )

            # Invoke with automatic fallback support
            result = self._invoke_with_fallback(model_idx=model_idx, messages=messages, **kwargs)
    ...
    def with_structured_output(self, schema: Type[BaseModel], **kwargs: Any) -> Runnable:
        ...
        def structured_output_func(messages: Union[List[BaseMessage], str]) -> BaseModel:
            ...
            logger.debug(
                f"Selected model {model_idx} for structured output (schema: {schema.__name__})"
            )
            ...
            if handler.has_native_support(selected_model):
                ...
                try:
                    ...
                    result = handler.invoke_native(
                        model=selected_model, schema=schema, messages=messages, **kwargs
                    )
